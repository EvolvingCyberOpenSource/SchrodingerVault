...
pub(crate) fn get_aes_key_ref<'a>(
...
pub(crate) fn zeroize_aes_key() -> Result<(), &'static str> {
    let mut guard = VAULT_AES_KEY.write().map_err(|_| "lock poisoned")?;
...
pub(crate) fn encrypt_password(aes_key: &[u8; 32], password_utf8: &str) -> Result<SealResult, String> {
...
pub(crate) fn remove_file_if_exists(p: &Path) -> io::Result<()> {
